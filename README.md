...
- Persistent Agent Chat dock that keeps conversation, project context, and Codex-facing response briefs inside the workbench. Natural-language text does not become a keyword-routed ticket or state mutation; execution uses explicit controls or future schema-validated agent proposals.
...
- User Settings v1 with top-right settings access, Light/Dark display themes, built-in `en-US` and `ja-JP` LocalePacks, LocalePack-driven app chrome/tabs/common controls/chat copy, dynamic local locale-pack registration with English fallback, password-authenticated server persistence for language/avatar/model preferences when auth is enabled, and localization AgentTask planning for future Codex-generated packs.
- Password Auth v0 with optional `TABLEX_AUTH_ENABLED=true`, first-user bootstrap, HTTP-only session cookies, logout, server-backed user preferences, and a 10+ character mixed-case/digit/symbol password policy. Google auth is planned behind the same harness-owned auth boundary.
...
- Data Review v1 with a harness-controlled DuckDB EDA runner over the uploaded DatasetSnapshot, producing human-readable HTML, SVG figures, JSON bundle, Markdown report, VisualizationSpec, Evidence, Insight, and lineage. It can be launched from explicit in-product controls without external dashboards or connector credentials.
