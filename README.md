...
- **Evaluation and modeling:** EvaluationSpec, SplitManifest, ExperimentRun, ModelVersion, and model diagnostics. Every registered model candidate must be completed with its own validated, versioned pipeline bundle that preserves its full inference path, runs from the UI, and downloads for local training and prediction; Full Auto does not finish by dropping candidates with missing bundles.
