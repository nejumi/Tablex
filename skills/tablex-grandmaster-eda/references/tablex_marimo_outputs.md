...
- For Japanese or other non-Latin labels, prefer Plotly/HTML-rendered text when it gives reliable glyph rendering in the Tablex viewer. When matplotlib or seaborn is the right tool for human-facing Japanese labels, import `japanize_matplotlib` in the notebook setup before drawing figures; Tablex exposes its availability in `python_runtimes.tablex_backend.packages.japanize_matplotlib`.
