...
Leaderboard 是预测就绪模型候选的晋升界面。每个已注册候选都必须完成为各自独立的可下载 pipeline，不能通过删除不完整候选来宣称完成。Full Auto 会持续运行，直到所有 bundle 通过隔离预测冒烟测试、能在同一依赖环境中启动训练入口、以数值精度匹配对应 run 的主要指标，并包含该模型完整的特征构建、已拟合 estimator、manifest、训练与预测入口、依赖和本地使用说明。只有分数的 run 仅在这项必需工作进行期间保留在实验历史中。
