...
def build_default_goal_text(db: Session, project: Project) -> str:
    ...
    return "\n".join(
        [
            ...
            "- Keep disposable ablations in a structured experiment ledger; promote serious distinct candidates to registered runs and complete their pipelines. Do not register every probe as a Leaderboard row, and never drop a run after registration.",
            "- Build and surface the foundation incrementally. Sanity models, application/main-table baselines, standard relational or temporal baselines, and material evidence-ladder steps are serious candidates, not disposable probes. Finish and register each candidate with its own downloadable smoke-tested pipeline as soon as its comparable evaluation is ready; do not leave the Leaderboard empty while batching the entire modeling study, final notebook, or diagnostics.",
            "- Keep each serious candidate vertically complete before moving far ahead: evaluation evidence, ExperimentRun, train/predict pipeline, and isolated smoke validation. Diagnostics and narrative may mature afterward without hiding a usable model. Do not back-load every registration and pipeline to the end of a long turn.",
