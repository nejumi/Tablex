...
- Leaderboard runs need stable `model_id`, human-readable `model_description`, `features_used`, and numeric metrics. Prefer one comparable primary metric across a result set.
- After registering runs, read the ack. If pipeline registration is missing or partial, continue by creating reproducible `pipelines/<name>/` directories and submitting `.tablex/requests/pipelines/` `register_prediction_pipeline` requests before treating modeling/reporting as complete.
