...
- An ExperimentRun is promoted to the Leaderboard only after its reproducible pipeline bundle passes isolated prediction smoke validation, reproduces its registered metrics, remains downloadable, and contains the local train/predict contract. After registering runs, read the ack. If pipeline registration is missing or partial, continue by creating reproducible `pipelines/<name>/` directories and submitting `.tablex/requests/pipelines/` `register_prediction_pipeline` requests before treating modeling/reporting as complete.
