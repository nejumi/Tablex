...
- Leaderboard membership is a promotion contract, not a synonym for a successful experiment. A run appears there only after a linked `prediction_pipeline` bundle passes isolated prediction smoke validation, starts its training entrypoint successfully in the same isolated dependency environment, matches that run's primary metric at numerical precision, and contains the local training, prediction, dependency, manifest, and usage files required for download. Score-only runs remain visible in experiment history until the bundle is ready.
