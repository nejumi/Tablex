...
- Leaderboard membership is a promotion contract, not a synonym for a successful experiment. Every registered model run must receive its own linked `prediction_pipeline` bundle; missing bundles are unfinished autonomous work, not a reason to drop or hide a candidate permanently. Full Auto must continue until each bundle passes isolated prediction smoke validation, starts its training entrypoint successfully in the same isolated dependency environment, matches that run's primary metric at numerical precision, and contains the model-specific feature construction, fitted estimator, local training, prediction, dependency, manifest, and usage files required for download. A score-only run remains visible in experiment history only while its bundle is being completed and cannot satisfy a completed Leaderboard or modeling deliverable.
