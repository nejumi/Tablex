...
- `credential_download`: whether plan-backed selective download is available for required or selected files.
...
Kaggle datasets remain credentialed, but Tablex can now run harness-owned access probes, file inventories, and selective required-file downloads. The probe reads `KAGGLE_API_TOKEN`, `KAGGLE_USERNAME`, and/or `KAGGLE_KEY` from the harness process environment or gitignored `.env`, calls the Kaggle competition file-list API, and stores a `kaggle_credential_probe.v1` artifact containing only secret-free status, HTTP status, credential source labels, and next actions. The inventory endpoint stores `kaggle_competition_file_inventory.v1` with file names, sizes, catalog role mapping, and missing required file summary. The selective download endpoint stores `kaggle_selective_download_manifest.v1` with downloaded file hashes, skipped reasons, local status, and import readiness. These endpoints never pass credential values to Codex, AgentRunner, AgentTaskContracts, runner workspaces, logs, or artifacts. Public UCI archives and selected OpenML CSV exports are credential-free and can be downloaded by the managed public-download endpoint when `source_card.access.supports_direct_download=true` and `requires_account=false`.
...
The probe, inventory, and selective download accept modern JSON or opaque `KAGGLE_API_TOKEN` values, `KAGGLE_USERNAME` plus `KAGGLE_API_TOKEN`, and legacy `KAGGLE_USERNAME` plus `KAGGLE_KEY`. If the API returns `forbidden_or_rules_required`, open the competition in Kaggle, accept the rules with the user account, and rerun the endpoint.
