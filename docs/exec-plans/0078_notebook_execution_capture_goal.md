...
Add the first harness-owned execution capture path for generated analysis notebooks. The product should persist runner evidence artifacts and native marimo/source-linked review paths without exposing secrets, connector credentials, external dashboards, or uncontrolled local files.
...
- Added integration coverage for manifest safety policy, static compile status, source registration, and figure manifest.
...
- Static compile confirms syntax only; it does not prove notebook cell runtime behavior. Runtime failures should surface as repair targets through native marimo, not as alternate snapshots.
