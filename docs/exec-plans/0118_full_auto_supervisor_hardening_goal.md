...
| Pending Chat turns should show whether Tablex has nudged Codex for a human-facing update | Implemented | Active Chat wait cards now include last Chat update age, progress-request-sent metadata, raw transcript counts, and the latest Codex message excerpt. The observation uses `AgentTranscriptEvent` when available and falls back to the saved Codex stdout JSONL when DB event ingestion is missing, without generating a replacement natural-language answer in the harness. |
