...
| Raw transcript storage must scale beyond small logs | Partially implemented | `(session_id, event_index)` indexes exist and stream events are batched. Future work should continue moving UI polling toward incremental-only reads everywhere. |
...
- Chat quality still depends on Codex writing `reports/chat_update.md` at a useful cadence. The harness requests and ingests those updates, but real long-running sessions should be observed to confirm the cadence feels natural.
