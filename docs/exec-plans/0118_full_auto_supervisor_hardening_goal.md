...
| Pending Chat turns should show whether Tablex has nudged Codex for a human-facing update | Implemented | Active Chat wait cards now include last Chat update age, progress-request-sent metadata, raw transcript counts, and the latest saved Codex message excerpt from `AgentTranscriptEvent`, without generating a replacement natural-language answer in the harness. |
