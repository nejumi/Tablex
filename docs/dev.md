...
`POST /api/projects/{project_id}/autonomy/start` starts a real agent-loop tick. It is not a mode-only write and must not behave like a ticket generator. The body accepts `autonomy_mode=approval_based|full_auto` and `runner_mode=harness_only|codex_cli|codex_cli_if_available`. The tick creates a `start_autonomous_loop` job, sets `current_phase=AUTONOMOUS_LOOP`, then advances every safe prerequisite it can: adaptive strategy, ResearchPlan, data quality, EDA review, data-understanding notebook, provisional target review, evaluation candidates, approval review, SplitManifest generation when an EvaluationSpec is approved, research brief, approach Ideas, insights, optional baseline/model runs when target/evaluation/split are ready, decision artifacts, an open-ended AgentTaskContract, a materialized runner workspace, readiness review, and an `autonomous_reflection` artifact. In `approval_based` mode it prepares target/evaluation review evidence but does not adopt the spec or train automatically; human approval remains the boundary. In `full_auto` mode, an unanswered target question is an intervention point, not a hard stop: Tablex records a Question, Evidence, and an Assumption with `fallback_policy=infer_and_continue`, then continues with a provisional target when the data profile supports one. The response includes `assistant_message`, `steps`, `interventions`, `boundaries`, `worker_events`, and `next_human_boundary` so Home can present one concise Agent update while the artifacts remain inspectable elsewhere. Large or already-understood projects must not recompute heavy EDA/notebooks, split manifests, or train models inside the Start request. Existing data-understanding artifacts are reused when present; SplitManifest generation is queued once the dataset exceeds `TABLEX_AUTONOMY_SYNC_SPLIT_ROW_LIMIT` rows, and model training is queued once the dataset exceeds `TABLEX_AUTONOMY_SYNC_TRAINING_ROW_LIMIT` rows. `runner_mode=codex_cli_if_available` prepares Codex workspaces without making Full Auto wait on a queued runner; use `runner_mode=codex_cli` only for an explicit synchronous runner invocation.
...
The intended primary workflow is Home-centered: create a project, upload data, optionally set or defer the target, choose Approval Based or Full Auto, then start. Data Understanding and plan creation are mandatory early steps. If the target is missing, Home/Agent Chat should ask while continuing with explicit assumptions in Full Auto. The UI intervention countdown is controlled by User Settings (`interventionCountdownSeconds`); `0` disables the dialog entirely. If the target must be derived, such as a binarized column, monthly aggregate, prediction interval, prediction distribution, or inverse-problem objective, the user should be able to discuss it with the agent; Tablex should create a target-construction artifact only through an explicit control or a schema-validated agent proposal before deployment-grade claims depend on it. After the initial plan, the agent may add, remove, or revise tasks autonomously or from user requests. Home should keep the plan list, current task, Agent panel, Ideas & Findings memory, and equipped Skill list visible; most users should not need to hunt across tabs during normal operation. The Agent panel supports a wrapped Tablex chat mode for human guidance and a Raw Codex-style mode for low-level chat/job/contract/event inspection without exposing secrets or connector credentials.
...
By default, the chat response composer attempts Codex CLI when it is available. If Codex cannot run, Tablex must say that plainly instead of returning a fake success message such as a saved-ticket summary. Target definition follows the same boundary: the harness may package profiles, semantic catalogs, assumptions, questions, EDA, and lineage into an AgentTaskContract, and Full Auto may adopt a provisional target only as an auditable assumption with confidence, risk, evidence, and fallback policy. Codex/AgentRunner can revise or replace that assumption through a schema-validated `target_definition_proposal`, which the harness can validate, register as Evidence/Assumption, and then use to update evaluation.
...
The worker uses concrete handlers for `build_split_manifest`, `run_baseline`, `train_model_candidates`, and `run_planned_agent_task_codex`; generic queued jobs still use MVP stub handlers until the async worker layer is expanded. When the split worker finishes a queued large-dataset SplitManifest, it queues the baseline and model-candidate training jobs using the generated manifest. Full Auto Start may queue long-running model work, but it must not make the main loop depend on a queued Codex job that no worker is currently processing. Agent Activity should show queued jobs as waiting, not live, and stale queued jobs should fall out of the right-edge activity overlay while remaining visible in Jobs/history.
