...
Data Understanding notebook generation accepts the user's response locale and stores it as `response_locale` on the notebook, HTML preview, report, run manifest, and authoring brief. The scaffold must label itself as `harness_scaffold_pending_codex_authoring`; it is not a substitute for the final Codex-authored marimo analysis. When Codex runs, the AgentSession context includes `human_interface.response_locale` and equipped Skill details so Codex can read `tablex-grandmaster-eda`, `tablex-notebook-quality`, project artifacts, and actual data before writing human-facing notebook narrative in the user's language.
