...
`POST /api/projects/{project_id}/autonomy/start` starts a real Full Auto autonomous-loop tick. It is not a mode-only write and must not behave like a ticket generator. The tick sets `autonomy_mode=full_auto`, creates a `start_autonomous_loop` job, then advances every safe prerequisite it can: adaptive strategy, ResearchPlan, data quality, EDA review, data-understanding notebook, evaluation candidates, non-destructive EvaluationSpec adoption when approval review passes, SplitManifest generation, research brief, approach Ideas, insights, optional baseline/model runs when target/evaluation/split are ready, decision artifacts, an open-ended AgentTaskContract, a materialized runner workspace, readiness review, and an `autonomous_reflection` artifact. The response includes `assistant_message`, `steps`, `boundaries`, `worker_events`, and `next_human_boundary` so Home can present one concise Agent update while the artifacts remain inspectable elsewhere.
The `runner_mode` body accepts `harness_only`, `codex_cli`, or `codex_cli_if_available`. The UI uses `codex_cli_if_available` for Full Auto. Codex execution still goes through AgentTaskContract, workspace materialization, readiness review, EvaluationSpec/SplitManifest discipline, no connector credential materialization, and artifact ingestion. Harness guidance is advisory context; Codex may accept, reject, revise, or replace suggested approaches and must emit artifact-backed outputs plus an approach decision trace.
