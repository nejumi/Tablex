...
The intended primary workflow is Home-centered: create a project, upload data, optionally set or defer the target, choose Approval Based or Full Auto, then start. Data Understanding and plan creation are mandatory early steps. If the target is missing, Home/Agent Chat should ask while continuing with explicit assumptions. If the target must be derived, such as a binarized column or monthly aggregate, the user should be able to discuss it with the agent; Tablex should create a target-construction artifact only through an explicit control or a schema-validated agent proposal before evaluation and modeling depend on it. After the initial plan, the agent may add, remove, or revise tasks autonomously or from user requests. Home should keep the plan list, current task, Agent panel, Ideas & Findings memory, and equipped Skill list visible; most users should not need to hunt across tabs during normal operation. The Agent panel supports a wrapped Tablex chat mode for human guidance and a Raw Codex-style mode for low-level chat/job/contract/event inspection without exposing secrets or connector credentials.
...
By default, the chat response composer attempts Codex CLI when it is available. If Codex cannot run, Tablex must say that plainly instead of returning a fake success message such as a saved-ticket summary. Target definition follows the same boundary: the harness may package profiles, semantic catalogs, assumptions, questions, EDA, and lineage into an AgentTaskContract, but it must not pick a prediction target with column-name or statistical shortcuts. Codex/AgentRunner should return a schema-validated `target_definition_proposal`, which the harness can validate, register as Evidence/Assumption, and then use to continue evaluation.
...
The worker uses concrete handlers for `run_baseline`, `train_model_candidates`, and `run_planned_agent_task_codex`; generic queued jobs still use MVP stub handlers until the async worker layer is expanded. Full Auto Start relies on these worker-visible queued jobs for long-running model and Codex work.
