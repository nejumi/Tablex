...
Post-run result reading should move through Experiments, Leaderboard, diagnostics, notebooks, and reports without making the user hunt. Agent Chat recognizes leaderboard and top-run comparison requests such as "リーダーボードを見せて" and "上位runを比較して". If no successful run exists, Chat routes back to Experiments. If runs exist, Chat routes to the Leaderboard Reader or creates an `experiment_comparison` artifact first. The Leaderboard Reader exposes top-run diagnostics, top-run report, and diagnostics notebook actions directly near the rank context so a user can turn a metric into evidence before a decision report. The Reader auto-previews the latest `experiment_comparison_report` when present, and its Post-run Report action runs a bounded `post_run_reading_workflow` that diagnoses the top run when possible, compares runs, generates the current decision report, and routes the user to Reports first. Missing diagnostics must be surfaced as evidence gaps instead of filled in with invented model evidence.
