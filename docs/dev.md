...
The Data Review endpoint runs harness-controlled DuckDB analysis over a DatasetSnapshot and stores `eda_review_bundle`, `eda_review_html`, `eda_review_svg`, `eda_review_report`, and `visualization_spec` artifacts plus Report, Evidence, Insight, and lineage. It computes shape, target status, missingness pressure, numeric median/IQR/outlier hints, categorical cardinality/top values, simple target relationships, numeric correlation candidates, findings, read order, story cards, a review playbook, and Codex next prompts. It does not execute user notebook code, access external networks, or materialize connector credentials. The Notebooks tab exposes this as `Run EDA Review`, and Agent Chat can route requests such as "run EDA", "visualize this data", or "データレビューを作って" to the same harness action.
...
`POST /api/projects/{project_id}/autonomy/start` starts a real agent-loop tick. It is not a mode-only write and must not behave like a ticket generator. The body accepts `autonomy_mode=approval_based|full_auto` and `runner_mode=harness_only|codex_cli|codex_cli_if_available`. The tick creates a `start_autonomous_loop` job, sets `current_phase=AUTONOMOUS_LOOP`, then advances every safe prerequisite it can: adaptive strategy, ResearchPlan, data quality, EDA review, data-understanding notebook, evaluation candidates, approval review, SplitManifest generation when an EvaluationSpec is approved, research brief, approach Ideas, insights, optional baseline/model runs when target/evaluation/split are ready, decision artifacts, an open-ended AgentTaskContract, a materialized runner workspace, readiness review, and an `autonomous_reflection` artifact. In `approval_based` mode it prepares EvaluationSpec review evidence but does not adopt the spec or train automatically; human approval remains the boundary. In `full_auto` mode it may adopt a non-blocked EvaluationSpec and continue. The response includes `assistant_message`, `steps`, `boundaries`, `worker_events`, and `next_human_boundary` so Home can present one concise Agent update while the artifacts remain inspectable elsewhere.
...
The intended primary workflow is Home-centered: create a project, upload data, optionally set or defer the target, choose Approval Based or Full Auto, then start. Data Understanding and plan creation are mandatory early steps. If the target is missing, Home/Agent Chat should ask while continuing with explicit assumptions. If the target must be derived, such as a binarized column or monthly aggregate, the user should be able to describe it in Agent Chat and Tablex should create a target-construction plan/artifact before evaluation and modeling. After the initial plan, the agent may add, remove, or revise tasks autonomously or from user requests. Home should keep the plan list, current task, Agent panel, Ideas & Findings memory, and equipped Skill list visible; most users should not need to hunt across tabs during normal operation. The Agent panel supports a wrapped Tablex chat mode for human guidance and a Raw Codex-style mode for low-level chat/job/contract/event inspection without exposing secrets or connector credentials.
Agent Chat is intentionally a human-facing buffer over the raw event stream. `/api/projects/{project_id}/agent-chat` stores both the user-facing assistant message and an `agent_human_response_brief.v1` payload that a future Codex/LLM response composer can read. The deterministic harness records intent, actions, artifacts, lineage, evaluation boundaries, and worker events; the composer is responsible for explaining the result in the user's language without sounding like a ticket log. `/api/projects/{project_id}/agent-chat/history` restores persisted chat turns from `agent_chat_turn` artifacts so the wrapped Chat view and Raw view do not diverge.
...
Post-run result reading should move through Experiments, Leaderboard, diagnostics, notebooks, and reports without making the user hunt. Agent Chat recognizes leaderboard and top-run comparison requests such as "リーダーボードを見せて" and "上位runを比較して". If no successful run exists, Chat routes back to Experiments. If runs exist, Chat routes to the Leaderboard Result Readout or creates an `experiment_comparison` artifact first. `/api/projects/{project_id}/results/readout` is a read-only aggregation of the current top run, EvaluationSpec/SplitManifest contract, diagnostics, comparison report, notebook gap, current decision report state, evidence gaps, and next action. The Leaderboard tab must show this readout before raw tables; raw rank rows, diagnostic artifact tables, and preview controls are supporting evidence behind disclosure. The Reader auto-previews the latest `experiment_comparison_report` when present, and its Post-run Report action runs a bounded `post_run_reading_workflow` that diagnoses the top run when possible, compares runs, generates the current decision report, and routes the user to Reports first. Missing diagnostics must be surfaced as evidence gaps instead of filled in with invented model evidence.
...
Agent Chat actions can include both `target_tab` and `target_anchor`. The UI uses them to switch to the correct tab and scroll to the exact surface such as `data-focus`, `relational-map`, `notebook-focus`, `analysis-story`, `evaluation-design`, or `approach-handoff`. Chat responses should route users to concrete surfaces whenever possible; broad tab-only navigation is a fallback, not the desired experience.
