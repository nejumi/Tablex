...
Agent Chat artifact actions should prefer the explicit `artifact_id` and fall back to the first `artifact_ids` entry when older or utility-produced actions only provide the list. This keeps Chat links openable even when a producer returns a compact artifact set instead of one primary id.
