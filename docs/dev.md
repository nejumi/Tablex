...
`POST /api/projects/{project_id}/autonomy/stop` powers the agent loop off by first persisting `current_phase=IDLE`, stopping the active main AgentSession, cancelling queued/running token-consuming jobs, terminating the project's Codex process groups, and cancelling durable child compute. In the supported split runtime, the container API writes a fixed-format stop request into the shared data control directory; the host supervisor terminates matching Codex process groups and acknowledges only after two consecutive zero-process observations. The request remains as a non-secret execution tombstone so a delayed worker cannot start Codex after power-off or project deletion; explicit Start clears it. The power-off state remains authoritative across worker and supervisor restarts; a late worker result cannot overwrite cancellation, and Agent Chat rejects new work until power is explicitly turned on again. If process or compute cleanup cannot be verified, the endpoint returns an error while keeping power off. Project deletion applies the same boundary, removes project-scoped compute records, metadata, artifacts, workspaces, and temporary response-composer directories, and returns `deleted=true` only after cleanup is verified. The selected `autonomy_mode` is preserved so the user can switch modes independently from Start/Stop. The Home UI treats the mode toggle as policy selection and the large power button as execution state. Tablee animates subtly while the loop is on and more actively when agent/model/notebook/research work is actually running. Harness guidance is advisory context; Codex may accept, reject, revise, or replace suggested approaches and must emit artifact-backed outputs.
...
On hosts with `nvidia-smi`, the launcher also builds `Dockerfile.gpu` through `docker-compose.gpu.yml` and accepts that runtime only after the resource detector observes a usable GPU and at least one library passes a real GPU probe. Otherwise it logs the reason and starts the ordinary CPU services. The trusted worker claims `run_agent_compute` and sends only a schema-validated execution request to `tablex-compute-executor` over an internal Docker network. Each request uses its Job id as a durable execution id: worker/executor restarts reattach or make a bounded restart attempt instead of silently losing or duplicating the run. Codex returns control while child compute runs, and the same main session resumes after the terminal acknowledgement is registered. The executor has no Codex auth mount, metadata database, external network, or published port; it uses a read-only root filesystem and a writable shared artifact root. The API shares only the internal executor network so power-off and deletion can cancel and verify compute cleanup. The worker then persists `compute_resource_evidence` plus output/log lineage. Do not infer GPU readiness from a device name or CUDA version alone.
